...
def login():
    ...
    # Verificar que no sean usuarios bloqueados
    blocked_users = ["alumno", "profesor"]
    
    if (username and password and 
        username != "User name" and password != "Password" and
        username.lower() not in blocked_users):
...
def create_cmd_icon():
    # Crear un icono de CMD programáticamente
    size = 40
    img = Image.new('RGB', (size, size), color='black')
    draw = ImageDraw.Draw(img)
    
    # Dibujar el símbolo > en blanco
    draw.text((8, 10), ">_", fill='white', font=None)
    
    return img

img = Image.new("RGBA", (32, 32), (0, 0, 0, 0))
img.save("transparent.ico")
...
# Icono de la aplicación - Siempre crear uno programáticamente
cmd_icon = create_cmd_icon()
icon_photo = ImageTk.PhotoImage(cmd_icon)
...
# Show more details (enlace) - ELIMINADO (logo de pluma)
...
# Domain label
domain_label = tk.Label(body_frame, text="Domain: CMD WINDOWS MICROSOFT",
                        bg=color_body, fg="black",
                        font=("Segoe UI", 9))
